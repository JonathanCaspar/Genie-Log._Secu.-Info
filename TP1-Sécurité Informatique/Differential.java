...
public class Differential{
	...
	public static String getPartialSubkey(){
		...
		for(int i = 0; i < 1000; i++){
			//Crꢴion de paires de messages clairs qui satisfont
			//la diff곥ntielle d'entrꥠ\Delta_P
		...
		//D굥rminer les boites de substitution impliqu꦳
		for(int i = 0; i < 4; i++) {
		...
		//D굥rminer les bits impliqu꦳ apr鳠permutation de la sortie des boites impliqu꦳
		for(int i = 0; i < 4; i++) {
		...
		for(int j = 0; j < 256; j++){
			...
			//j possible nous donne la diff곥ntielle intermꥩaire 
			//"int_diff" ࡣounts[j]
			String jBinary = toBinary(j, 16);			
						
			for(String cipher : ciphers) {
				// Restriction du cipher aux sections concern꦳ 
				/*
				String cipher8bits = "";
				...
				//								 =
				//						1 1		  000   100 => � faire permuter vers le haut et substitution pour comparer avec intDiff
		...
		//D굥rminer la fr겵ence de clef la plus haute
		int max = 0;
		int maxIndex = -1;
		...
		//D굥rminer la sous-clef k_5^* avec des 'X' au bits inconnues
